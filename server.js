...
// Códigos más largos: 12 caracteres en lugar de 8
  ...
  res.send(layout('Sellador de Documentos', `
    ...
    <form action="/upload" method="post" enctype="multipart/form-data" class="card">
      ...
      <div class="seal-config">
        ...
        <div class="controls">
          <div class="grid3">
            <input type="radio" name="sealPosition" id="pos0" value="top-left">
            <label for="pos0">↖️</label>
            
            <input type="radio" name="sealPosition" id="pos1" value="top-center">
            <label for="pos1">⬆️</label>
            
            <input type="radio" name="sealPosition" id="pos2" value="top-right">
            <label for="pos2">↗️</label>
            
            <input type="radio" name="sealPosition" id="pos3" value="middle-left">
            <label for="pos3">⬅️</label>
            
            <input type="radio" name="sealPosition" id="pos4" value="middle-center">
            <label for="pos4">●</label>
            
            <input type="radio" name="sealPosition" id="pos5" value="middle-right">
            <label for="pos5">➡️</label>
            
            <input type="radio" name="sealPosition" id="pos6" value="bottom-left">
            <label for="pos6">↙️</label>
            
            <input type="radio" name="sealPosition" id="pos7" value="bottom-center">
            <label for="pos7">⬇️</label>
            
            <input type="radio" name="sealPosition" id="pos8" value="bottom-right" checked>
            <label for="pos8">↘️</label>
          </div>
    ...
    <script>
      ...
      function updateOverlay() {
        ...
        const sizeConfig = { small: { qr: 45 }, medium: { qr: 65 }, large: { qr: 90 } };
        const selectedSize = document.querySelector('input[name="sealSize"]:checked')?.value || 'medium';
        const selectedPos = document.querySelector('input[name="sealPosition"]:checked')?.value || 'bottom-right';
        ...
        const blockWidth = cfg.qr + 8 + 130;
        const blockHeight = cfg.qr + 100;
        const pageWidth = canvas.offsetWidth;
        const pageHeight = canvas.offsetHeight;
        const marginPt = 20;
        
        const positions = {
          'top-left': { x: marginPt, y: pageHeight - marginPt - blockHeight },
          'top-center': { x: (pageWidth - blockWidth) / 2, y: pageHeight - marginPt - blockHeight },
          'top-right': { x: pageWidth - marginPt - blockWidth, y: pageHeight - marginPt - blockHeight },
          'middle-left': { x: marginPt, y: (pageHeight - blockHeight) / 2 },
          'middle-center': { x: (pageWidth - blockWidth) / 2, y: (pageHeight - blockHeight) / 2 },
          'middle-right': { x: pageWidth - marginPt - blockWidth, y: (pageHeight - blockHeight) / 2 },
          'bottom-left': { x: marginPt, y: marginPt },
          'bottom-center': { x: (pageWidth - blockWidth) / 2, y: marginPt },
          'bottom-right': { x: pageWidth - marginPt - blockWidth, y: marginPt }
        };
        
        const box = positions[selectedPos] || positions['bottom-right'];
      ...
      document.querySelectorAll('input[name="sealPosition"], input[name="sealSize"]').forEach(input => {
...
app.post('/upload', (req, res) => {
  ...
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      // Texto del sello (VERTICAL: QR arriba, texto abajo)
      ...
      const lines1 = wrapText(line1, font, cfg.font, cfg.qr);
      const lines2 = wrapText(line2, fontBold, cfg.font, cfg.qr);
      const lines3 = wrapText(line3, font, cfg.font, cfg.qr);
      const lines4 = wrapText(line4, font, cfg.font, cfg.qr);
      ...
      const allLines = [
        ...
        ...lines4.map(t => ({ text: t, bold: false }))
      ];

      // SELLO VERTICAL: QR al tope, texto abajo
      const totalTextHeight = allLines.length * lineGap;
      const blockHeight = cfg.qr + GAP_PT + totalTextHeight + 8;
      const blockWidth = Math.max(cfg.qr, cfg.textWidth);

      for (const page of pages) {
        const { width, height } = page.getSize();
        const box = computeBoxPosition(sealPosition, width, height, blockWidth, blockHeight, MARGIN_PT);

        // QR arriba (centrado horizontalmente)
        const qrX = box.x + (blockWidth - cfg.qr) / 2;
        const qrY = box.y + blockHeight - cfg.qr;
        page.drawImage(qrImage, { x: qrX, y: qrY, width: cfg.qr, height: cfg.qr });

        // Texto abajo del QR
        let textY = box.y + blockHeight - cfg.qr - GAP_PT - cfg.font;
        const textX = box.x + (blockWidth - cfg.qr) / 2;
        
        for (const line of allLines) {
          const lineWidth = (line.bold ? fontBold : font).widthOfTextAtSize(line.text, cfg.font);
          const centeredX = textX + (cfg.qr - lineWidth) / 2;
