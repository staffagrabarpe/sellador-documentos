  ...
  res.send(layout('Sellador de Documentos', `
    ...
    <script>
      ...
      function updateOverlay() {
        ...
        const sizeConfig = { small: { qr: 45, textWidth: 100 }, medium: { qr: 65, textWidth: 150 }, large: { qr: 90, textWidth: 200 } };
        const selectedSize = document.querySelector('input[name="sealSize"]:checked')?.value || 'medium';
        
        if (!canvas.offsetParent) return;
        
        const cfg = sizeConfig[selectedSize];
        const blockWidth = cfg.qr + 8 + cfg.textWidth;  // QR + gap + texto (horizontal)
        const blockHeight = cfg.qr + 20;  // Altura base del QR más un poco de margen
  ...
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      for (const page of pages) {
        ...
        // Calcular dimensiones del bloque (QR izquierda + texto derecha)
        ...
        const qrY = sealYPt + (blockHeight - cfg.qr) / 2;
        const qrX = sealXPt;
        page.drawImage(qrImage, { x: qrX, y: qrY, width: cfg.qr, height: cfg.qr });

        // TEXTO a la DERECHA del QR (centrado verticalmente)
        let textY = sealYPt + (blockHeight - totalTextHeight) / 2 + totalTextHeight - cfg.font;
        const textX = sealXPt + cfg.qr + GAP_PT;
