...
// ---- Configuracion de tamanos del sello (QR vertical + bloque de texto debajo) ----
const SIZE_CONFIG = {
  small:  { qr: 45, textWidth: 80, font: 6 },
  medium: { qr: 65, textWidth: 120, font: 7 },
  large:  { qr: 90, textWidth: 140, font: 8 }
};
const GAP_PT = 6;
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  #seal-overlay{
    ...
    border-radius:4px;display:none;align-items:center;justify-content:center;font-size:9px;color:var(--primary-dark);text-align:center;line-height:1.1;cursor:grab;user-select:none
  }
  ...
  res.send(layout('Sellador de Documentos', `
    ...
    <script>
      ...
      function updateOverlay() {
        ...
        const sizeConfig = { small: { qr: 45, textWidth: 80 }, medium: { qr: 65, textWidth: 120 }, large: { qr: 90, textWidth: 140 } };
        const selectedSize = document.querySelector('input[name="sealSize"]:checked')?.value || 'medium';
        
        if (!canvas.offsetParent) return;
        
        const cfg = sizeConfig[selectedSize];
        const blockWidth = cfg.textWidth;
        const blockHeight = cfg.qr + 6 + (cfg.textWidth * 2);
        
        const sealX = parseFloat(document.getElementById('sealX').value) || 20;
        const sealY = parseFloat(document.getElementById('sealY').value) || 20;
      ...
      document.addEventListener('mousemove', (e) => {
        ...
        const overlay = document.getElementById('seal-overlay');
        const maxX = canvas.offsetWidth - overlay.offsetWidth;
        const maxY = canvas.offsetHeight - overlay.offsetHeight;
      ...
      document.querySelector('input[name="file"]').addEventListener('change', async function() {
        ...
        try {
          ...
          hint.textContent = 'Arrastra el sello para posicionarlo en el documento. El contenido del QR y textos se generan al enviar.';
          updateOverlay();
        } catch (err) {
          console.error(err);
...
app.post('/upload', (req, res) => {
  ...
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      // Texto del sello (VERTICAL: Texto rotado arriba, QR abajo)
      ...
      for (const page of pages) {
        ...
        // Coordenadas ajustadas a puntos PDF (de pixels a puntos)
        const scale = 72 / 96; // Aproximación de conversión
        const sealXPt = sealX * scale;
        const sealYPt = sealY * scale;

        // Calcular dimensiones del bloque
        const blockWidth = cfg.textWidth;
        const blockHeight = totalTextHeight + GAP_PT + cfg.qr;

        // QR centrado abajo (en puntos)
        const qrX = sealXPt + (blockWidth - cfg.qr) / 2;
        const qrY = sealYPt;
        page.drawImage(qrImage, { x: qrX, y: qrY, width: cfg.qr, height: cfg.qr });

        // Texto arriba del QR, rotado 90 grados
        let textY = sealYPt + cfg.qr + GAP_PT + cfg.font;
        const textX = sealXPt + blockWidth / 2;
        
        for (const line of allTextLines) {
          const textFont = line.bold ? fontBold : font;
          const lineWidth = textFont.widthOfTextAtSize(line.text, cfg.font);
          const centeredX = textX - lineWidth / 2;
