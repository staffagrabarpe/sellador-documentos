...
// Migracion suave por si la tabla ya existia sin estas columnas
...
// ---- Tamanos y posiciones del sello (compartido con el JS del cliente) ----
const SIZE_PT = { small: 50, medium: 80, large: 120 };
const MARGIN_PT = 20;

function computeSealBox(positionCode, pageWidth, pageHeight, sizePt, marginPt) {
  const s = sizePt;
  const m = marginPt;
  const positions = {
    'top-left':       { x: m,                         y: pageHeight - m - s },
    'top-center':     { x: (pageWidth - s) / 2,        y: pageHeight - m - s },
    'top-right':      { x: pageWidth - m - s,          y: pageHeight - m - s },
    'middle-left':    { x: m,                          y: (pageHeight - s) / 2 },
    'middle-center':  { x: (pageWidth - s) / 2,         y: (pageHeight - s) / 2 },
    'middle-right':   { x: pageWidth - m - s,           y: (pageHeight - s) / 2 },
    'bottom-left':    { x: m,                           y: m },
    'bottom-center':  { x: (pageWidth - s) / 2,         y: m },
    'bottom-right':   { x: pageWidth - m - s,           y: m }
  };
  return positions[positionCode] || positions['bottom-right'];
}

// ---- Plantilla base / estilos ----
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  #seal-overlay{
    ...
    border-radius:4px;display:none;align-items:center;justify-content:center;font-size:11px;color:var(--primary-dark);pointer-events:none
  }
  .controls{flex:1;min-width:240px}
  .grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;max-width:220px}
  .grid3 label{
    border:1px solid var(--border);border-radius:8px;aspect-ratio:1;display:flex;align-items:center;justify-content:center;
    cursor:pointer;background:#fbfbfe;font-size:18px;user-select:none
  }
...
// ---------- Pagina principal: subir + configurar sello ----------
app.get('/', (req, res) => {
  res.send(layout('Sellar documento', `
    ...
    <form action="/upload" method="post" enctype="multipart/form-data" class="card" id="sealForm">
      ...
      <div class="seal-config">
        ...
        <div class="controls">
          ...
          <div class="grid3">
            <input type="radio" name="sealPositionRadio" id="p-tl" value="top-left">
            <label for="p-tl">↖</label>
            <input type="radio" name="sealPositionRadio" id="p-tc" value="top-center">
            <label for="p-tc">↑</label>
            <input type="radio" name="sealPositionRadio" id="p-tr" value="top-right">
            <label for="p-tr">↗</label>

            <input type="radio" name="sealPositionRadio" id="p-ml" value="middle-left">
            <label for="p-ml">←</label>
            <input type="radio" name="sealPositionRadio" id="p-mc" value="middle-center">
            <label for="p-mc">•</label>
            <input type="radio" name="sealPositionRadio" id="p-mr" value="middle-right">
            <label for="p-mr">→</label>

            <input type="radio" name="sealPositionRadio" id="p-bl" value="bottom-left">
            <label for="p-bl">↙</label>
            <input type="radio" name="sealPositionRadio" id="p-bc" value="bottom-center">
            <label for="p-bc">↓</label>
            <input type="radio" name="sealPositionRadio" id="p-br" value="bottom-right" checked>
            <label for="p-br">↘</label>
          </div>

          <label class="field-label" style="margin-top:18px">Tamaño del sello</label>
          <div class="sizes">
            <input type="radio" name="sealSizeRadio" id="s-s" value="small">
            <label for="s-s">Pequeño</label>
            <input type="radio" name="sealSizeRadio" id="s-m" value="medium" checked>
            <label for="s-m">Mediano</label>
            <input type="radio" name="sealSizeRadio" id="s-l" value="large">
            <label for="s-l">Grande</label>
          </div>

          <p class="muted" style="margin-top:14px">El sello (QR + código) se colocará en <b>todas las páginas</b> del documento.</p>
        </div>
      </div>
    ...
    <script>
      ...
      function updateOverlay() {
        ...
        const sizePt = SIZE_PT[size];
        const box = computeSealBox(position, currentPageSize.width, currentPageSize.height, sizePt, MARGIN_PT);

        const leftPx = box.x * scale;
        const topPx = (currentPageSize.height - box.y - sizePt) * scale;
      ...
      document.getElementById('fileInput').addEventListener('change', async (e) => {
        ...
        try {
          ...
          const displayScale = Math.min(500 / viewport.width, 1.4);
          ...
          hint.textContent = 'Así se verá aproximadamente el sello (el QR real se genera al enviar el formulario).';
          updateOverlay();
        } catch (err) {
          console.error(err);
...
app.post('/upload', (req, res) => {
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      const sealPosition = (req.body.sealPosition && computeSealBox(req.body.sealPosition, 100, 100, 10, 1)) ? req.body.sealPosition : 'bottom-right';
      const sealSize = SIZE_PT[req.body.sealSize] ? req.body.sealSize : 'medium';
      const sizePt = SIZE_PT[sealSize];
      ...
      for (const page of pages) {
        ...
        const box = computeSealBox(sealPosition, width, height, sizePt, MARGIN_PT);
        ...
        const fontSize = Math.max(6, Math.round(sizePt * 0.09));
        page.drawText(`Codigo: ${code}`, {
          x: box.x,
          y: Math.max(2, box.y - fontSize - 2),
          size: fontSize,
          font,
      ...
      db.prepare(`INSERT INTO documents (code, original_name, password_hash, doc_hash, seal_position, seal_size, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .run(code, req.file.originalname, passwordHash, docHash, sealPosition, sealSize, new Date().toISOString());
      ...
      res.send(layout('Documento sellado', `
        ...
        <div class="card">
          ...
          <p style="color:var(--err)"><b>⚠️ Guarda esta contraseña ahora, no se volverá a mostrar.</b></p>
...
app.get('/validar/:code', (req, res) => {
  ...
  res.send(layout('Validar documento', `
    ...
    <p class="lead">Ingresa la contraseña que te compartieron junto con este código.</p>
    <p><span class="badge">Código</span> <span class="mono">${code}</span></p>
